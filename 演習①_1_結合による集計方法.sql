...
SELECT 
    ...
    , MAX(mhi.hinmoku_name) AS hinmoku_name    -- 品目マスタ.品目名称
    , SUM(thu.uriage) AS uriage_sum            -- 日別売上.売上（合計）
    , ROUND(AVG(thu.uriage),0) AS uriage_avg   -- 日別売上.売上（平均）
    , MIN(thu.uriage) AS uriage_min            -- 日別売上.売上（最小）
    , MAX(thu.uriage) AS uriage_max            -- 日別売上.売上（最大）
...
, ts_dt AS (
SELECT 
      thu.hanbai_date AS hanbai_date           -- 日別売上.販売日
    , SUM(thu.uriage) AS uriage_sum            -- 日別売上.売上（合計）
    , ROUND(AVG(thu.uriage),0) AS uriage_avg   -- 日別売上.売上（平均）
    , MIN(thu.uriage) AS uriage_min            -- 日別売上.売上（最小）
    , MAX(thu.uriage) AS uriage_max            -- 日別売上.売上（最大）
...
SELECT 
    ...
    , ts_dt.uriage_sum     AS "全品目の日別合計"        -- 日別売上を集計（日別）.売上（合計）
    , ts_hi.uriage_sum     AS "品目別で全期間の合計"     -- 日別売上を集計（品目単位）.売上（合計）
    , ts_to.uriage_sum     AS "全品目・全期間の合計"     -- 日別売上を集計（総合計）.売上（合計）
FROM t_hibetsu_uriage thu                            -- 日別売上

LEFT OUTER JOIN mhi                                  -- 品目マスタ
ON thu.hinmoku_cd = mhi.hinmoku_cd                   -- 品目コードで結合

LEFT OUTER JOIN ts_dt                                -- 日別売上を集計（日別）
ON thu.hanbai_date = ts_dt.hanbai_date               -- 販売日で結合

LEFT OUTER JOIN ts_hi                                -- 日別売上を集計（品目単位）
ON thu.hinmoku_cd = ts_hi.hinmoku_cd                 -- 品目コードで結合

CROSS JOIN ts_to                                     -- 日別売上を集計（総合計）
